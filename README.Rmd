...
For `devtools`, installation requires systeme dependent tools. They must be installed first.  
...
### SticsRFiles
`SticsRFiles` must be installed manually, remotely using the above syntax, just replacing  **SticsOnR** with **SticsRFiles**. 
...
Here we reuse the model executable path defined prevously with respect to the operating system (Windows, Linux or Mac) and the directory where individual Usms input directories have been generated.
