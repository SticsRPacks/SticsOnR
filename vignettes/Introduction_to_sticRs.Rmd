...
The different steps a [STICS](https://www6.paca.inra.fr/stics_eng/) developper (or user) often experience can be summarized as:
...
* Use different formalisms in STICS (or new ones for developpers), and then evaluate them using observations
...
The function returns a list of two objects : `outputs` and `gg_object`. The first one contains the model outputs, and the second one the automatic plot.
The plot returned by the function is accessible as follows:
```{r eval=FALSE}
eval_parameter$gg_object
```
And it returns this plot: 
...
1. The model version, which plots the model simulation output as lines. The model runs (here one for each parameter value) are differenciated with the line type.

1. The observation source, which plots the observations present on the simulation folder as points (source folder is always the same using `stics_eval`, but not necessarily using `plot_output`). The sources are differenciated with the point type.

1. The plant dominance, which plots the plant dominance at initialization (*i.e.* Principal or Associated) as colors. This plant dominance is different from the `dominant` output, which tracks the day-to-day plant dominance. This output only have one value for sole crop simulations.


The `outputs` object contains the model outputs for each STICS run, which are named after the parameter that was tested, or the model version that was tested, and stored as list. For our previous example, it would have two model outputs, one for the simulation with `P_laicomp= 0.1`, and one for `P_laicomp= 0.4`:
```{r eval=FALSE}
...
The values are accessible as follows:
```{r eval=FALSE}
...
```
Each `outputs` object is a data.frame with the output variables asked with the `Out_var` parameter of `stics_eval`, along with the plant dominance (both plants in the same data.frame), the Posixct date, the year, month, day and DOI named after the model output (*i.e.* ian, mo, jo and jul respectively), and all measured variables found in the observation files. The simulated and measured variables are differenciated using the "_sim" and "_meas" suffixes respectively. 

These data.frames look like this (print only the 9 first columns, 4 first rows):
```{r echo=FALSE}
...
The plot returned by the function is as follows for model version comparison:
```{r eval=FALSE}
...
```
It is the same as for parameter comparison, except that the "Model Version" legend shows the model version instead of the parameter used for simulation.
