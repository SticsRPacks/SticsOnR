---
title: "Introduction to SticsOnR"
author: "Rémi Vezy"
date: "`r Sys.Date()`"
output:
  rmarkdown::html_vignette:
    fig_caption: yes
vignette: >
  %\VignetteIndexEntry{Introduction to SticsOnR}
...
The [SticsOnR](https://SticsRPacks.github.io/SticsOnR/) package provides tools for [STICS](https://www6.paca.inra.fr/stics_eng/) developers and users.  
...
This vignette is meant to introduce the concepts of STICS development and parameterization using the [SticsOnR](https://SticsRPacks.github.io/SticsOnR/) package.
...
It is strongly encouraged to use your own data instead of the dummy data set. The [SticsOnR](https://SticsRPacks.github.io/SticsOnR/) package uses the above-mentioned files, so if you want to use your own data, you simply have to create a USM using javaSTICS for example, and to make a first run with it to create all files needed. After completion, it is encouraged to copy the folder to a new location before using [SticsOnR](https://SticsRPacks.github.io/SticsOnR/), or to use a version control system such as GIT to avoid overwriting a USM when re-opening javaSTICS.
...
There are two useful functions in the SticsOnR package to parameterize STICS inputs: `read_param` to read and `set_param` to modify a value of a parameter. both functions are wrapper of more specialized functions such as `read_plant` or `set_plant`, which read and set the plant parameters respectively.
...
The package also has a function to make sensitivity analyses: `sensitive_stics`. Its usage is detailed in a [separate vignette](https://SticsRPacks.github.io/SticsOnR/articles/Sensitivity_analyses.html) for a clearer description.
