compare_2usms <- function(first_dir,second_dir,in_usms_list=c()){
  ...
  usms_list=base :: intersect(SticsRFiles::get_usms_list(first_dir_path),
                              SticsRFiles::get_usms_list(second_dir_path))
  ...
  plants_nb=SticsRFiles::get_plants_nb(first_dir_path)
  plants_nb_2=SticsRFiles::get_plants_nb(second_dir_path)
  ...
  for (usm in 1:usms_nb){
    ...
    for (plant in 1:nplants){
      ...
      if (methods :: is(first_tbl,"try-error")) next
      second_tbl <- try(read.table(second_path,header = TRUE,sep = ";",stringsAsFactors = FALSE))
      if (methods :: is(second_tbl,"try-error")) next
      ...
      for (var in 1:length(common_names)){
        ...
        cor_val=try(stats :: cor(val_first,val_second))
        if (methods :: is(cor_val,"try-error")) {
          print(varname)
        } else {
          cor_var[var]=cor_val
        }
