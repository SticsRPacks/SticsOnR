...
set_stics_exe <- function(javastics, stics_exe, overwrite= FALSE,verbose=TRUE) {
  ...
  if(check_stics_exe(model_path = java_stics_exe, stop = FALSE)){
    ...
    is_copied <- file.copy(from = stics_exe, to = java_stics_exe,
                           overwrite = overwrite)
    if(!is_copied){
      stop("Impossible to copy stics_exe file into the bin directory of
  ...
  if(exist_stics_name){
    ...
    if(stics_exe_list$stics_list[[new_stics_name]]!=exe_file_name){
      ...
      while(exist_stics_exe(javastics, new_stics_name)){
        i <- i + 1
        new_stics_name <- paste0(gsub(".exe","",exe_file_name),
                                 "_",user_os(),"_",i)
      }
    }else{
      # If they have the same executable, use the same name.
      SticsRFiles:::setValues(xml_pref,
  ...
  stics_exe_string <- paste0(sprintf("{%s\t%s},",
                                     names(stics_exe_list$stics_list),
                                     stics_exe_list$stics_list), collapse = "")
  xml_pref <- SticsRFiles:::xmldocument(xml_path)
  SticsRFiles:::setValues(xml_pref,
                          '//entry[@key="model.last"]', new_stics_name)
  SticsRFiles:::setValues(xml_pref,
...
list_stics_exe <- function(javastics){
  ...
  if(!is_pref){
    ...
    current_stics <- SticsRFiles:::getValues(xml_pref,
...
select_stics_exe <- function(javastics, stics_exe= "stics_modulo"){
  ...
  xml_pref <- SticsRFiles:::xmldocument(xml_path)
  current_model <- SticsRFiles:::getValues(xml_pref,
                                           '//entry[@key="model.last"]')
...
remove_stics_exe <- function(javastics,stics_exe){
  ...
  stics_exe_list$stics_list <- stics_exe_list$stics_list[-grep(stics_exe,
                                              names(stics_exe_list$stics_list))]
